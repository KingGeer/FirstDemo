...
form_VerifyList.push(new Array("verify_loginname", "", "4-16位含字母、数字、_、."));
form_VerifyList.push(new Array("verify_pwd", "", "6-20个字符"));
...
form_VerifyList.push(new Array("verify_cnen", "", "只能为全中文2-4或全英文2-30"));
...
function verify_loginname(obj, tips, msg) {
    ...
    //4-20个字符，一个汉字为两个字符，推荐使用中文会员名。一旦注册成功会员名不能修改。
    if (v_len < 4 || v_len > 20) {
        set_flag_verify(obj, "会员名在4-20个字符内。", tips, false);
        return false
    } else {
        var v_userName = $(obj).val();
        if (/^\d*$/.test(v_userName)) {
            set_flag_verify(obj, "会员名不能全为数字。", tips, false);
            return false
        } else {
            var chines = /^[^!@#$%^&*()\-=+]+$/;
            if (!chines.test(v_userName)) {
                set_flag_verify(obj, "非法的会员名。", tips, false);
                return false
            }
        }
    }
...
function verify_pwd(obj, tips, msg) {
    ...
    if (v_obj_val.length < 6 || v_obj_val.length > 20) set_flag_verify(obj, msg, tips, false);
    else set_flag_verify(obj, msg, tips, true);
}
function verify_pwdA(obj, tips, msg) {
...
//只能为全中文2-4或全英文2-30
function verify_cnen(obj, tips, msg) {
    var reg = /^[\u4e00-\u9fa5]{2,4}$|^[a-zA-Z]{2,30}$/;
